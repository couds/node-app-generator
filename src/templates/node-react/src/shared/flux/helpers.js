...
export const genericRetryStrategy = ({ maxRetryAttempts = 10, scalingDuration = 1000 } = {}) => {
  return (attempts$) => {
 ...
 */
const createRequestEpic = (type, params) => {
  return (action$, state$) => {
    ...
    const actions = {
      ...
      cancel: (action) => {
        return { type: handlerNames.cancel, context: action };
      },
      finish: (action) => {
        return { type: handlerNames.finish, context: action };
      },
      progress: (action) => {
        return { type: handlerNames.progress, context: action };
      },
      retry: (action, attempt) => {
        return { ...action, __attempt: attempt + 1, type: handlerNames.retry };
      },
    };
    return action$.pipe(
      ...
      mergeMap((action) => {
        ...
        const ajax$ = from(request).pipe(
          map((data) => {
            return actions.success(data.data, action);
          }),
          catchError((error) => {
